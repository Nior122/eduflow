  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground">{data?.className ? `Weekly schedule — ${data.className}` : "Weekly schedule"}</p>
      </div>
      ...
      {!data || data.days.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-muted-foreground text-sm">Your class timetable will appear here once published.</p>
        </div>
      ) : (
              ...
              <CardContent className="space-y-2">
                {d.entries.map((e) => (
                  <div key={e.id} className="flex items-center gap-3 rounded-lg border p-3">
                    ...
                    <Badge variant="secondary" className="shrink-0">{e.startTime}–{e.endTime}</Badge>
                  </div>
                ))}
              </CardContent>
            </Card>
